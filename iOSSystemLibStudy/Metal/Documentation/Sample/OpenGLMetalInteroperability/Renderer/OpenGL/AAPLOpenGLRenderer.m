...
- (void)useInteropTextureAsBaseMap:(GLuint)name
{
    ...
    if(_textureDimensionIndex < 0)
    {
        assert(!"No textureDimensions uniform in rectangle texture fragment shader");
    }
    ...
    
    {
        ...
        if(!texInfo || error)
        {
            ...
            assert(!"Error: could not get texture from url");
        }
                    ...
                    withFragmentSourceURL:(NSURL*)fragmentSourceURL
{
    ...
    if(!vertSourceString || error)
    {
        NSLog(@"Could not load vertex shader source: %@", error.localizedDescription);
        return 0;
    }
    ...
    if(!fragSourceString || error)
    {
        NSLog(@"Could not load fragment shader source: %@", error.localizedDescription);
        return 0;
    }
    ...
    if(_mvpUniformIndex < 0)
    {
        assert(!"Could not get MVP Uniform Index");
    }

    {
        ...
        if(samplerLoc < 0)
        {
            assert(!"Could not get sampler Uniform Index");
        }
