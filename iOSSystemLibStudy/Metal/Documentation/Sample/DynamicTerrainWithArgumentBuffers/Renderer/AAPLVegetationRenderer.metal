...
vertex VegetationVertexOut vegetation_vertex(   device const AAPLObjVertex* in [[ buffer(0) ]],
                                                device const float4x4* instances [[ buffer(1) ]],
                                                ...
                                                uint2 tid                                       [[thread_position_in_grid]])
{
    ...
    if (pop_idx < kPopulationCount)
    {
        ...
        float wind_speed =  0.1 + 0.4 * saturate(1.0 + (cos(globalUniforms.gameTime * 0.8 + nrnd1 * 1.0 + world_pos.z * -0.0002 + sin(world_pos.x * -0.001))));
        float windx = (1.0 + 0.2 * sin(globalUniforms.gameTime * 20.0f * (nrnd0-.5))) * wind_speed;
        float windy = (1.0 + 0.2 * cos(globalUniforms.gameTime * 20.0f * (nrnd0-.5))) * wind_speed;
