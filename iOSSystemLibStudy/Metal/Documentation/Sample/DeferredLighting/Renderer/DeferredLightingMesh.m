...
@implementation DeferredLightingMesh {
                                       ...
                                       error:(NSError * __nullable * __nullable)error
{
    ...
    // Create an array to hold this DeferredLightingMesh object's AAPLSubmesh objects
...
+ (NSArray<DeferredLightingMesh*> *) newMeshesFromObject:(nonnull MDLObject*)object
                     modelIOVertexDescriptor:(nonnull MDLVertexDescriptor*)vertexDescriptor
                       metalKitTextureLoader:(nonnull MTKTextureLoader *)textureLoader
                                 metalDevice:(nonnull id<MTLDevice>)device
                                       error:(NSError * __nullable * __nullable)error {

    NSMutableArray<DeferredLightingMesh *> *newMeshes = [[NSMutableArray alloc] init];

    // If this ModelIO  object is a mesh object (not a camera, light, or something else)...
    if ([object isKindOfClass:[MDLMesh class]])
    {
        //...create an app-specific DeferredLightingMesh object from it
        MDLMesh* mesh = (MDLMesh*) object;

        DeferredLightingMesh *newMesh = [[DeferredLightingMesh alloc] initWithModelIOMesh:mesh
    ...
    // of this ModelIO  object and create app-specific DeferredLightingMesh objects from those ModelIO meshes
    for (MDLObject *child in object.children)
    {
        NSArray<DeferredLightingMesh*> *childMeshes;

        childMeshes = [DeferredLightingMesh newMeshesFromObject:child
                            modelIOVertexDescriptor:vertexDescriptor
                              metalKitTextureLoader:textureLoader
                                        metalDevice:device
                                              error:error];
...
+ (nullable NSArray<DeferredLightingMesh *> *) newMeshesFromURL:(nonnull NSURL *)url
                            modelIOVertexDescriptor:(nonnull MDLVertexDescriptor *)vertexDescriptor
                                        metalDevice:(nonnull id<MTLDevice>)device
                                              error:(NSError * __nullable * __nullable)error
{
    ...
    // DeferredLightingMesh objects from those ModelIO meshes
    for(MDLObject* object in asset)
    {
        NSArray<DeferredLightingMesh *> *assetMeshes;

        assetMeshes = [DeferredLightingMesh newMeshesFromObject:object
                            modelIOVertexDescriptor:vertexDescriptor
                              metalKitTextureLoader:textureLoader
                                        metalDevice:device
                                              error:error];
