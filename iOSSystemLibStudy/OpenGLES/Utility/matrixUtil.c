...
void mtxLoadLookAt(float *mtx, const float *eyePosition, const float *target, const float *viewUp)
{
    float forward[3];
    vec3Subtract(forward, target, eyePosition);
    vec3Normalize(forward, forward);
    
    float side[3];
    vec3CrossProduct(side, forward, viewUp);
    vec3Normalize(side, side);
    
    float up[3];
    vec3CrossProduct(up, side, forward);
    vec3Normalize(up, up);
    ...
    //第四列
    mtx[12] = -vec3DotProduct(side, eyePosition);
    mtx[13] = -vec3DotProduct(up, eyePosition);
    mtx[14] = vec3DotProduct(forward, eyePosition);
    mtx[15] = 1.0f;
}
