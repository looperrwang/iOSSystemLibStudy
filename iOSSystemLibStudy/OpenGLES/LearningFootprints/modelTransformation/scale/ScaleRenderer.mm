...
- (void)render
{
    ...
    //绘制第二个矩形
    mtxLoadIdentity(model);
    mtxScaleMatrix(model, 0.5f, 0.5f, 1.0f);
    mtxTranslateMatrix(model, -0.25f, 0.0f, 0.0f);
    glUniformMatrix4fv(glGetUniformLocation(_modelShader->_programId, "model"), 1, GL_FALSE, model);
    glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_SHORT, 0);
    
    //绘制第三个矩形
    mtxLoadIdentity(model);
    mtxScaleMatrix(model, 2.0f, 2.0f, 1.0f);
    mtxTranslateMatrix(model, -1.0f, -1.0f, 0.0f);
    glUniformMatrix4fv(glGetUniformLocation(_modelShader->_programId, "model"), 1, GL_FALSE, model);
    glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_SHORT, 0);
    
    //绘制第四个矩形
    mtxLoadIdentity(model);
    mtxScaleMatrix(model, 2.0f, 0.5f, 1.0f);
    mtxTranslateMatrix(model, 0.0f, -0.25f, 0.0f);
    glUniformMatrix4fv(glGetUniformLocation(_modelShader->_programId, "model"), 1, GL_FALSE, model);
    glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_SHORT, 0);
