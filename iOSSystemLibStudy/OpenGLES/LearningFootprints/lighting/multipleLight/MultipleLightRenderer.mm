...
@interface MultipleLightRenderer ()
...
@implementation MultipleLightRenderer
...
- (void)initGLResource
{
    ...
    NSString *modelVertexPath = [[[NSBundle mainBundle] bundlePath] stringByAppendingPathComponent:@"resource/OpenGLES/LearningFootprints/lighting/multipleLight/shaders/cube.vert"];
    NSString *modelFragPath = [[[NSBundle mainBundle] bundlePath] stringByAppendingPathComponent:@"resource/OpenGLES/LearningFootprints/lighting/multipleLight/shaders/cube.frag"];
    _modelShader = new Shader(modelVertexPath.UTF8String, modelFragPath.UTF8String);
    
    NSString *lampVertexPath = [[[NSBundle mainBundle] bundlePath] stringByAppendingPathComponent:@"resource/OpenGLES/LearningFootprints/lighting/multipleLight/shaders/lamp.vert"];
    NSString *lampFragPath = [[[NSBundle mainBundle] bundlePath] stringByAppendingPathComponent:@"resource/OpenGLES/LearningFootprints/lighting/multipleLight/shaders/lamp.frag"];
...
- (void)render
{
    ...
    //传递光照数据
    GLfloat lampDir[3] = {0.5f, 0.8f, 0.0f};
    glUniform3f(glGetUniformLocation(_modelShader->_programId, "dirLight.ambient"), 0.0f, 0.1f, 0.4f);
    glUniform3f(glGetUniformLocation(_modelShader->_programId, "dirLight.diffuse"), 0.0f, 0.2f, 0.8f);
    glUniform3f(glGetUniformLocation(_modelShader->_programId, "dirLight.specular"), 0.0f, 1.0f, 1.0f);
    glUniform3f(glGetUniformLocation(_modelShader->_programId, "dirLight.direction"), lampDir[0], lampDir[1], lampDir[2]);
    
    for (int i = 0; i < sizeof(pointLightPositions) / (sizeof(pointLightPositions[0] * 3)); ++i) {
        std::stringstream indexStr;
        indexStr << i;
        std::string lightName = "pointLights[" + indexStr.str() + "]";
        GLint lightAmbientLoc = glGetUniformLocation(_modelShader->_programId, (lightName + ".ambient").c_str());
        GLint lightDiffuseLoc = glGetUniformLocation(_modelShader->_programId, (lightName + ".diffuse").c_str());
        GLint lightSpecularLoc = glGetUniformLocation(_modelShader->_programId, (lightName + ".specular").c_str());
        GLint lightPosLoc = glGetUniformLocation(_modelShader->_programId, (lightName + ".position").c_str());
        GLint attConstant = glGetUniformLocation(_modelShader->_programId, (lightName + ".constant").c_str());
        GLint attLinear = glGetUniformLocation(_modelShader->_programId, (lightName + ".linear").c_str());
        GLint attQuadratic = glGetUniformLocation(_modelShader->_programId, (lightName + ".quadratic").c_str());
        ...
        glUniform3f(lightPosLoc, pointLightPositions[i * 3], pointLightPositions[i * 3 + 1], pointLightPositions[i * 3 + 2]);
    ...
    glUniform3f(glGetUniformLocation(_modelShader->_programId, "spotLight.ambient"), 0.0f, 0.1f, 0.4f);
    glUniform3f(glGetUniformLocation(_modelShader->_programId, "spotLight.diffuse"), 0.0f, 0.2f, 0.8f);
    glUniform3f(glGetUniformLocation(_modelShader->_programId, "spotLight.specular"), 0.0f, 1.0f, 1.0f);
    glUniform3f(glGetUniformLocation(_modelShader->_programId, "spotLight.position"), eyePos[0], eyePos[1], eyePos[2]);
    glUniform3f(glGetUniformLocation(_modelShader->_programId, "spotLight.direction"), target[0] - eyePos[0], target[1] - eyePos[1], target[2] - eyePos[2]);
    glUniform1f(glGetUniformLocation(_modelShader->_programId, "spotLight.cutoff"), cosf(12.5f * M_PI / 180.0f));
    glUniform1f(glGetUniformLocation(_modelShader->_programId, "spotLight.outerCutoff"), cosf(17.5f * M_PI / 180.0f));
    
    glUniform1f(glGetUniformLocation(_modelShader->_programId, "spotLight.constant"), 1.0f);
    glUniform1f(glGetUniformLocation(_modelShader->_programId, "spotLight.linear"), 0.09f);
    glUniform1f(glGetUniformLocation(_modelShader->_programId, "spotLight.quadratic"), 0.032f);
    ...
    _lampShader->use();
    glUniformMatrix4fv(glGetUniformLocation(_lampShader->_programId, "projection"),
                       1, GL_FALSE, projection);
    glUniformMatrix4fv(glGetUniformLocation(_lampShader->_programId, "view"),
                       1, GL_FALSE, view);
    for (int i = 0; i < sizeof(pointLightPositions) / (sizeof(GLfloat) * 3); ++i) {
        mtxLoadIdentity(model);
        mtxScaleMatrix(model, 0.2f, 0.2f, 0.2f);
        mtxTranslateMatrix(model, pointLightPositions[i * 3], pointLightPositions[i * 3 + 1], pointLightPositions[i * 3 + 2]);
        glUniformMatrix4fv(glGetUniformLocation(_lampShader->_programId, "model"),
                           1, GL_FALSE, model);
        glDrawArrays(GL_TRIANGLES, 0, 36);
    }
